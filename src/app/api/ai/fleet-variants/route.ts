import { parseAiVariantRequest } from "../../../../features/experiment-fleet/lib/ai-variants.ts";
import { generateVariantCandidates } from "../../../../features/experiment-fleet/server/generate-variant-candidates.ts";
import { createRateLimiter, hasAllowedOrigin, isCrossSite, jsonResponse, readBoundedBody } from "../../../../shared/server/request-guards.ts";
import { isAiProviderRuntimeEnabled } from "../diagnosis/route.ts";
...
export async function POST(request: Request): Promise<Response> {
  ...
  const parsed = parseAiVariantRequest(value);
  ...
  const result = await generateVariantCandidates(parsed.value, {
