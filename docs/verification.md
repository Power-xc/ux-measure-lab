...
| Fleet 측정 | segments 측정 2종 · FLEET-PREFILL-001~003 · segmented reader 3종 | 변형별 퍼널 정규화(값 사전순·미달 변형 결손 통지), provenance dimension 검증, 프리필의 결손·위조 거부, RPC 행 재조립·그룹 붕괴 거부, 첫 배정 귀속 마이그레이션 규칙 |
| Fleet 확정·AI 후보 | FLEET-PLAN-007 · FLEET-NEXT-006 · FLEET-WAVE-008 · FLEET-AI-001~005 | holdout 범위·예약 예산 검증, 확정 웨이브 통과·강등·재수집 분기, 단일 후보 웨이브 강제, AI 후보의 수치·판정·미허용 근거 거부와 결정적 fallback |
...
| DEPLOY-004 | PASS | `/api/ai/diagnosis`·`/api/ai/fleet-variants`는 production에서 항상 deterministic fallback |
